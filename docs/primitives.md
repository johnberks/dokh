...
- `/dev/primitives` é o catálogo interno, disponível apenas em builds de desenvolvimento. Há um botão temporário na Home provisória para abri-lo sem digitar deep link. Demonstra estados normal/desabilitado/loading/erro e labels de acessibilidade; em release o botão some e a rota redireciona ao início. O HTML `design/componentes.dc.html` documenta Review Card, não estas primitives; formas iniciais seguem os tokens e as telas de referência.
