...
| 7.1 Splash e tela de criar conta | 🟡 Splash animado de ~1,4 s e tela 04 com prévia em profundidade; carrossel e `Pular` removidos a pedido do usuário | Validar no iPhone (visual, VoiceOver e reduzir movimento); Android depois. O fluxo aprovado tem uma tela após o splash: a DoD original da 7.1 (três slides) foi substituída por decisão do usuário |
| 7.3 Perfil e bifurcação de Residência | 🟡 Telas 06, 07, 09, TELA 04 e 12 implementadas, com lista oficial de residências e gravação no Supabase | Validar no iPhone com Supabase local (gravação do perfil e da bolsa); Android depois |
