...
Feedback visual subsequente no iPhone: e-mail e senha cortavam letras com descendente enquanto a pessoa digitava. A causa era `TextInput` interno com 20 px de altura/linha no iOS. Ajustado para 28/24 px com 2 px de padding vertical, mantendo campo externo de 54 px, Archivo 15 px e paleta do HTML; teste de componente cobre os estilos. `npm run typecheck`, `npm run check`, `npm test -- --runInBand` (32 suítes/182 testes), Expo Doctor (21/21) e export iOS passaram. Aguardar conferência visual no aparelho antes de afirmar resolução completa.
