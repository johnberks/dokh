...
Feedback visual subsequente no iPhone: e-mail e senha cortavam letras com descendente enquanto a pessoa digitava. A primeira tentativa aumentou o `TextInput` interno de 20/20 para 28/24 px, mas a captura do usuário mostrou que o recorte continuava **somente com o teclado aberto**; sem foco, o texto fica correto. A revisão remove altura e `lineHeight` explícitas do `TextInput` de linha única, deixando a medida nativa do iOS; o campo externo continua com 54 px, Archivo 15 px e a paleta do HTML. Teste de componente verifica a ausência das restrições e preservação do rascunho. Aguardar nova conferência visual no aparelho antes de afirmar resolução completa.
