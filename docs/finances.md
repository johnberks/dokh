...
- ★ **Gráfico em bloco** (`BarChartCard`, `src/components`, superfície de card como o `CalendarCard`): título `GANHOS DE 2026` em negrito e espaçado, legenda `mês atual`, barras em sálvia cheio relativas ao maior mês, mês atual em bronze e mês sem dado **tracejado** (nunca zero). A média mensal é o rodapé do próprio bloco.
