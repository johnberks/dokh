...
- Se o cadastro exibir **Não foi possível conectar ao servidor**, conferir no Safari do próprio iPhone `http://<IP-LAN-DO-MAC>:54321/auth/v1/health`. O Expo Go abrir não prova que a porta 54321 do Docker está alcançável. Checar também se o IP do Mac ainda corresponde a `EXPO_PUBLIC_SUPABASE_URL` no `.env.local`, se o Supabase local está ativo e se Mac/iPhone estão no mesmo Wi-Fi. Reiniciar o Metro com `--clear` após mudar `.env.local`. Não registrar e-mail/senha/token durante o diagnóstico.
